...
안 될까? 간단히 설명하자면 프록시는 Dl에 기반을 두고 있고， 위의 코드는 Dl를
...
자동)  client → proxy(target) ←x client2


### 프록시의 다른종류 
**클래스프록시**  

위의 것은 DI를 이용해 했지만  client → target 인터페이스 없이 만들 수 있다. 
target의 상속구조를 사용하여  인터페이스처럼 사용함  

조건 1) final클래스 및 메소드 사용불가 2)CGLib이라는 바이트코드 생성 라이브러리가 필요하다.      
